...
HOVER_COLOR = (200, 200, 255)  # Color when hovering over the start options
BLUR_COLOR = (0, 0, 0, 128)  # Semi-transparent black for blur effect
...
def draw_selection_screen():
    ...
    if 0 < mouse_x < WIDTH // 2 and 0 < mouse_y < HEIGHT:
        pygame.draw.rect(screen, HOVER_COLOR, (0, 0, WIDTH // 2, HEIGHT), 5)
    if WIDTH // 2 < mouse_x < WIDTH and 0 < mouse_y < HEIGHT:
        pygame.draw.rect(screen, HOVER_COLOR, (WIDTH // 2, 0, WIDTH // 2, HEIGHT), 5)
...
def display_winner(winner):
    font = pygame.font.Font(None, 60)
    text = font.render(f"{'Red' if winner == 1 else 'Green'} Wins!", True, TEXT_COLOR)
    screen.blit(text, (WIDTH // 2 - text.get_width() // 2, HEIGHT // 3))
    
    # Add a blurred effect (semi-transparent black background)
    pygame.draw.rect(screen, BLUR_COLOR, (0, 0, WIDTH, HEIGHT))
    
    restart_msg = pygame.font.Font(None, 40).render("Click or Press R to Restart", True, TEXT_COLOR)
    screen.blit(restart_msg, (WIDTH // 2 - restart_msg.get_width() // 2, HEIGHT // 1.5))
    pygame.display.update()
